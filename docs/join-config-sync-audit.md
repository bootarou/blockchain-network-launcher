...
**追加修正（2026-08-18）**: 当初、生成処理が `fetchAndWritePeerFiles()` の内部に置かれていました。設定の出どころは `.ui-meta.json`（ローカルファイル）なのに、適用が**参加元への到達性に依存**する状態で、参加元が落ちていると正しく設定したスケジュールが黙って無視されていました。`patchInflationConfig()` として切り出し、Step 4c2 で無条件に実行するよう修正しています。
