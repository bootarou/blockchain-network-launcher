...
> 2026-09-22 訂正: 以下は当時の調査記録です。「最新の統計をコピーする」対策と「epoch 1 固定が原因」という結論は撤回しました。初期ブロックのハッシュと統計が一致すれば、Catapult は epoch 0 への逆探索に入りません。最新統計だけをコピーすると、epoch 205 の参加先では未取得の `00204.proof` を参照して停止します。現在は BlockElement の EntityHash から高さ1のハッシュ索引・証明・統計を揃えて生成し、REST 再構築経路のバイナリ形式も修正しています。バックアップ復旧後の既存データは上書きしません。
