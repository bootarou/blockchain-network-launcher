...
> ⚠️ **重要：** パスワードは十分に強力なものを設定してください。短いパスワードや単純な文字列はブルートフォース攻撃に脆弱です。
...
`docker-compose.yml` では、管理ポートはデフォルトで **localhost のみ** にバインドされています：

```yaml
ports:
  - "127.0.0.1:4000:4000"   # Backend API（localhost のみ）
  - "127.0.0.1:5173:5173"   # Frontend dev server（localhost のみ）
```

この設定により、VPS 上であっても管理画面は外部から直接アクセスできません。リモートからアクセスする場合は **SSH トンネル** を使用してください：
...
| **localhost バインド**（デフォルト） | `docker-compose.yml` のポート設定 | 外部から管理画面に直接アクセス不可 |
...
LAN 内の他の PC からアクセスしたい場合は、`docker-compose.yml` のポート設定を変更して `ADMIN_PASSWORD` を必ず設定してください：

```yaml
ports:
  - "4000:4000"   # 全インターフェースに公開（要認証）
```
