...
Symbol Network Manager の管理画面（ポート 4000）は、デフォルトでは **認証なし** で動作します。ローカル環境での利用を前提としていますが、VPS やクラウド上で運用する場合は **管理パスワードによるログイン認証** を有効にすることを強く推奨します。

### なぜ必要か

管理画面からはノードの停止・初期化・設定変更など、すべての操作が実行できます。ポート 4000 がインターネットに公開された状態で認証がない場合、第三者にノードを停止・破壊される危険があります。
...
デフォルトでは、管理ポートは **localhost のみ** にバインドされています。`.env` の `BIND_ADDRESS` で制御できます：
...
ssh -L 4000:localhost:4000 user@your-vps-ip
...
| **SSH トンネル** | `ssh -L 4000:localhost:4000 user@vps` | 暗号化されたリモートアクセス |
| **ファイアウォール** | `ufw deny 4000/tcp` | 管理ポートの追加ブロック |
